...
from pyokapi.folio import FolioService
from pyokapi.folio.api.configuration import Config
from pyokapi.folio.api.dataImport import DataImport
from pyokapi.folio.api.dataImportConverterStorage import \
    DataImportConverterStorage
from pyokapi.folio.api.inventoryStorage import InstanceStatus
from pyokapi.folio.exceptions import UnknownFile
from pyokapi.okapi.exceptions import OkapiRequestUnprocessableEntity

log = logging.getLogger("pyokapi.folio.dataImport")
...
class DataImportService(FolioService):
    ...
    def _create_jobProfile(self, marcType="MARC_BIBLIOGRAPHIC",
        ...
        name = f"pyokapi_{action}_{marcType}_{folioRecordType}_"
        jobProfiles = self._dataImportConverterStorage.get_jobProfiles(
            query=f"name==pyokapi*Job")
        if len(jobProfiles["jobProfiles"]) > 0:
        ...
        jobProfile = self._dataImportConverterStorage.set_jobProfile(
            {"profile": {"name": name+"Job",
                         "description": "pyokapi jobProfile to import files",
                         "dataType": "MARC"},
