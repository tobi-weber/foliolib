...
from foliolib.folio.api_deprecated.inventoryStorage import (AuthoritiesReindex,
                                                            AuthorityNoteType)
from foliolib.helper import is_valid_uuid, jprint
from foliolib.okapi.exceptions import OkapiRequestError

log = logging.getLogger("foliolib.folio.inventory")


class InventoryImpl(FolioAPIImpl):
    """Implentations of inventory related functions.
    """
    ...
    def __init__(self, tenant: str) -> None:
        ...
        self._search = Search(tenant)
        self._instanceReindex = InstanceReindex(tenant)
        self._authoritiesReindex = AuthoritiesReindex(tenant)
    ...
    def reindex(self):
        ...
        log.info("Reindex")
        try:
            res = self._search.createindices({"resourceName": "instance"})
            print("Instance index created.")
        except OkapiRequestError as e:
            log.debug("Instance index exists.")
        try:
            res = self._search.createindices(
                {"resourceName": "instance_subject"})
            log.info("Instance subject index created.")
        except OkapiRequestError as e:
            log.debug("Instance subject index exists.")
        try:
            res = self._search.createindices({"resourceName": "authority"})
            log.info("Authority index created.")
        except OkapiRequestError as e:
            log.debug("Authority index exists.")
        log.info("Reindex Inventory")
        res = self._search.reindexinventoryrecords({"recreateIndex": True})
        jprint(res)
        log.info("Reindex Instance")
        res = self._instanceReindex.set_reindex()
        jprint(res)
        try: #  Deprecated
            log.info("Reindex Authorities.")
            res = self._authoritiesReindex.set_reindex()
            jprint(res)
...
class InventoryReferenceData(FolioAPIImpl):
    ...
    def load_reference_data(self, path, replace=False):
        ...
        log.info("%i files failed.", failed)
        return count
