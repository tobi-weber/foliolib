...
log = logging.getLogger("foliolib.folio.api.kbEbscoJava")
    ...
    def get_packages_by_provider(self, providerId: str, **kwargs):
        ...
        Keyword Args:
            ...
            filter[type] (str): (default=all) Filter to narrow down results based on content type. Defaults to all.
                    Possible values are all, aggregatedfulltext, abstractandindex, ebook, ejournal, print, unknown, onlinereference.
...
class Tags(FolioApi):
    ...
    def get_tags(self, **kwargs):
        ...
        Keyword Args:
            filter[rectype]? (recordTypes):  Filter tags by one or more record types. Several types can be specified in form of `filter[rectype]=provider&filter[rectype]=package` Possible values are `provider`, `package`, `title`, `resource`
