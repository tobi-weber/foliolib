...
from foliolib import SRCPATH

if TYPE_CHECKING:
    from foliolib.raml.ramlMethod import RamlMethod


FILESPATH = os.path.join(SRCPATH, "docs", "source", "files")
...
def get_doc(methodName: str, method: 'RamlMethod', methodSig: str, dataParam: str = None):
    data = method.get_data()
    ...
    if dataParam is not None:
        ...
        if "example" in requests:
            ...
            with open(os.path.join(FILESPATH, requestExampleFile), "w") as f:
    ...
    if returns is not None:
        if "type" in returns:
            ...
            with open(os.path.join(FILESPATH, returnTypeFile), "w") as f:
                f.write(returnType)
            if "example" in returns:
                if "value" in returns["example"]:
                ...
                with open(os.path.join(FILESPATH, returnExampleFile), "w") as f:
